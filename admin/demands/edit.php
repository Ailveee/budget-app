...
// Securely get user session data
$userId = isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : null;
$userRole = isset($_SESSION['user_role']) ? strtolower($_SESSION['user_role']) : 'user';

// Redirect to login if not authenticated
if (!$userId) {
    header('Location: /auth/login.php'); // Adjust if login.php is elsewhere
    exit;
}

// Validate demand ID from query string
$demandId = $_GET['id'] ?? null;
if (!$demandId || !is_numeric($demandId)) {
    $redirect = ($userRole === 'admin') ? '../admin/demands/index.php' : '../user/index.php';
...
// Redirect if demand not found
if (!$demand) {
    die('Demand not found.');
}

// Access control: allow only the creator or admin
if ($userRole !== 'admin' && $demand['user_id'] != $userId) {
    die('Access denied.');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $newStatus = $demand['status']; // Default to current status
    ...
    // Budget check (only if admin is accepting the demand now)
    ...
    if (empty($errors)) {
        ...
        $_SESSION['message'] = $userRole === 'admin'
            ? "Demand updated successfully."
            : "Your demand has been updated.";

        $redirect = ($userRole === 'admin') ? '/budget_app/admin/demands/index.php' : '/budget_app/user/index.php';
        header("Location: $redirect");
        exit;
    }
}
...
<main class="container">
    ...
    <form method="post" action="">
        ...
        <input type="number" step="0.01" min="0.01" id="amount" name="amount"
               value="<?= htmlspecialchars($demand['amount']) ?>" required>

        <label for="demand_date">Demand Date:</label>
        <input type="date" id="demand_date" name="demand_date"
               value="<?= htmlspecialchars($demand['demand_date']) ?>" required>
    ...
    <p>
        <a href="delete_demand.php?id=<?= $demandId ?>" onclick="return confirm('Are you sure you want to delete this demand?');">
            Delete Demand
        </a>
    </p>

    <p>
        <a href="<?= $userRole === 'admin' ? '/budget_app/admin/demands/index.php' : '/budget_app/user/index.php'; ?>">
